...
- [x] **F1** `Background` now animates only when it's actually on screen: an `active` prop (App passes `onHome`) plus the Page Visibility API (`visibilitychange`) gate a single `sync()` that starts/stops the rAF loop. Off-home the routed screen slides over the fixed canvas, so there's no point drawing; the loop is also self-terminating (re-checks the gate before each reschedule). The animation machinery is built once and the prop only flips a ref-read flag, so pausing off-home and resuming on return keeps sprig positions instead of reseeding. No IntersectionObserver — the canvas is `position:fixed; inset:0`, so it always intersects the viewport and `active` is the real signal. Colocated browser test (`Background.test.tsx`, 4 tests) pins animate-while-active, never-start-inactive, stop-on-going-inactive, resume-on-reactivate. `56 tests` green (was 52).
